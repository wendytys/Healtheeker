(function ($) {
    "use strict";
    $.visualmodoElementsTrigger = $.visualmodoElementsTrigger || {};
    $.visualmodoElementsTrigger.wp_media = $.visualmodoElementsTrigger.wp_media || [];
    $.visualmodoElementsTrigger.media_new = function () {
        var $body = $("body");
        $body.on('click', '.visualmodo-elements-upload-icon', $.visualmodoElementsTrigger.media_new_activate);
    };
    //intended for zip files only. if needed should be easy to expand in the future
    $.visualmodoElementsTrigger.media_new_activate = function (event) {
        ...
        file_frame.on('select update insert', function () {
            $.visualmodoElementsTrigger.media_new_insert(file_frame, options);
        });
        //open the media frame
        file_frame.open();
    };
    //insert the url of the zip file
    $.visualmodoElementsTrigger.media_new_insert = function (file_frame, options) {
    ...
    $(document).ready(function () {
        $.visualmodoElementsTrigger.media_new();
        //Fonts Zip file upload
        $("body").on('visualmodo_elements_insert_zip', $.visualmodoElementsTrigger.icon_insert);
        //font manager
        $("body").on('click', '.visualmodo_elements_del_icon', $.visualmodoElementsTrigger.icon_remove);
    });
    /************************************************************************
     EXTRA FUNCTIONS, NOT NECESSARY FOR THE DEFAULT UPLOAD
     *************************************************************************/
    $.visualmodoElementsTrigger.icon_insert = function (event, selection, options) {
        // clean the options field, we dont need to save a value
        options.input_target.val("");
        var manager = $('.visualmodo_elements_iconfont_manager');
        var msg = $('#msg');
        ...
        $.ajax({
            ...
            url: ajaxurl,
            data: {
                action: 'visualmodo_elements_ajax_add_zipped_font',
                values: selection,
            },
            beforeSend: function () {
                $('.spinner').css({
            ...
            success: function (response) {
                $('.spinner').hide();
                if (response.match(/visualmodo_elements_font_added/)) {
    ...
    $.visualmodoElementsTrigger.icon_remove = function (event) {
        event.preventDefault();
        var button = $(this),
            parent = button.parents('.visualmodo-elements-available-font:eq(0)'),
            manager = button.parents('.visualmodo_elements_iconfont_manager:eq(0)'),
            all_fonts = manager.find('.visualmodo-elements-available-font'),
        ...
        $.ajax({
            ...
            url: ajaxurl,
            data: {
                action: 'visualmodo_elements_ajax_remove_zipped_font',
                del_font: del_font,
            },
            beforeSend: function () {
                $('.spinner').css({
            ...
            success: function (response) {
                $('.spinner').hide();
                if (response.match(/visualmodo_elements_font_removed/)) {
