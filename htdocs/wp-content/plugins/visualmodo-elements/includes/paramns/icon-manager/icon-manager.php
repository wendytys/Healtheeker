...
if ( ! class_exists( 'VISUALMODO_ELEMENTS_IPM' ) ) {
	class VISUALMODO_ELEMENTS_IPM {
		var $paths = array();
		var $svg_file;
		var $json_file;
		var $ip_name = 'unknown';
		...
		function __construct() {
			...
			$this->paths['temp']    = trailingslashit( $this->paths['fonts'] ) . 'visualmodo_elements_temp';
			...
			add_action( 'wp_ajax_visualmodo_elements_ajax_add_zipped_font', array( $this, 'add_zipped_font' ) );
			add_action( 'wp_ajax_visualmodo_elements_ajax_remove_zipped_font', array( $this, 'remove_zipped_font' ) );
			
		}
		
		function admin_scripts() {
			$upload_paths = wp_upload_dir();
			wp_enqueue_script( 'visualmodo-elements-admin-media', plugin_dir_url( __FILE__ ) . 'assets/js/icon-manager.min.js', array( 'jquery' ) );
			wp_enqueue_script( 'media-upload' );
			wp_enqueue_media();
			wp_enqueue_style( 'visualmodo-elements-icon-manager-admin', plugin_dir_url( __FILE__ ) . 'assets/css/icon-manager.css' );
			$custom_fonts = get_option( 'visualmodo_elements_ip' );
			if ( is_array( $custom_fonts ) ) {
				foreach ( $custom_fonts as $font => $info ) {
					if ( strpos( $info['style'], 'http://' ) !== false ) {
						wp_enqueue_style( 'visualmodo-elements-' . $font, $info['style'], null, '1.0', 'all' );
					} else {
						wp_enqueue_style( 'visualmodo-elements-' . $font, trailingslashit( $upload_paths['baseurl'] . '/visualmodo_elements_ip/' ) . $info['style'], null, '1.0', 'all' );
					}
				}
			}
		}
		...
		public function get_font_manager( $id ) {
			...
			$output .= '<ul class="icons-list visualmodo_elements_icon icon-list-' . $id . '">';
		...
		function icon_pack_manager() {
			...
			<div class="wrap">
			<h2>
			<?php esc_html_e( 'Icon Pack Manager', 'visualmodo' ); ?>
			<a href="#visualmodo_elements_upload_icon" class="button button-primary visualmodo-elements-upload-icon" data-target="iconfont_upload"
			data-title="<?php echo esc_html__( "Upload/Select Fontello Font Zip", "visualmodo" ) ?>"
			data-type="application/octet-stream, application/zip"
			data-button="<?php echo esc_html__( "Insert Fonts Zip File", "visualmodo" ); ?>" data-trigger="visualmodo_elements_insert_zip"
			data-class="media-frame ">
			<?php esc_html_e( 'Add New Icon Pack', 'visualmodo' ); ?>
			...
			if ( is_array( $fonts ) ) :
				...
				<div class="postbox visualmodo-elements-postbox-search">
				<h2 class="hndle ui-sortable-handle"><span>Search for Icons</h2>
				<div class="inside">
				<div class="search-area">
				<input class="search-icon" type="text" placeholder="<?php esc_attr_e( 'Start typing to search', 'visualmodo' ); ?>"/>
				...
				<div class="error">
				<p>
				<?php esc_html_e( 'No icon pack uploaded. Upload some icon pack to display here.', 'visualmodo' ); ?>
				</p>
				</div>
				<?php
			endif;
		...
		static function get_ipm() {
			...
			foreach ( $fonts as $font => $info ) {
				...
				$output .= '<div class="postbox visualmodo-elements-postbox-icon-manager">';
				include( $file );
				if ( ! empty( $icons ) ) {
					$icon_set = array_merge( $icon_set, $icons );
				}
				if ( ! empty( $icon_set ) ) {
					...
					if ( $n != 1 ) {
						$output .= '<button class="button button-primary button-small visualmodo_elements_del_icon" data-delete=' . esc_attr( $font ) . ' data-title="' . esc_attr__( 'Remove Pack', 'visualmodo' ) . '">' . esc_html__( 'Remove Pack', 'visualmodo' ) . '</button>';
					}
					$output .= '</h3>';
					$output .= '<div class="inside"><div class="icon_actions">';
					$output .= '</div>';
					$output .= '<div class="icon_search"><ul class="icons-list visualmodo_elements_icon">';
					foreach ( $icon_set as $icons ) {
						foreach ( $icons as $icon ) {
			...
			jQuery(document).ready(function(){
				jQuery(".search-icon").keyup(function(){
					...
					jQuery(".icons-list li").each(function(){
						...
						if(count == 0)
						jQuery(".search-count").html(" ' . esc_js( __( 'Can\'t find the icon?', 'visualmodo' ) ) . ' <a href=\'#visualmodo_elements_upload_icon\' class=\'button button-primary visualmodo_elements_upload_icon\' data-target=\'iconfont_upload\' data-title=\' ' . esc_js( __( 'Upload/Select Fontello Font Zip', 'visualmodo' ) ) . ' \' data-type=\'application/octet-stream, application/zip\' data-button=\' ' . esc_js( __( 'Insert Fonts Zip File', 'visualmodo' ) ) . ' \' data-trigger=\'visualmodo_elements_insert_zip\' data-class=\'media-frame\'>' . esc_js( __( 'Upload New Font Pack', 'visualmodo' ) ) . '</a>");
						else
						jQuery(".search-count").html(count+" ' . esc_js( __( 'Icons found.', 'visualmodo' ) ) . '");
		...
		function add_zipped_font() {
			...
			if ( ! current_user_can( $cap ) ) {
				die( esc_html__( "Using this feature is reserved for Super Admins. You unfortunately don't have the necessary permissions.", "visualmodo" ) );
			}
			...
			if ( $this->ip_name == 'unknown' ) {
				...
				die( esc_html__( 'Was not able to retrieve the Icon Pack name from your Uploaded Folder', 'visualmodo' ) );
			}
			die( esc_html__( 'visualmodo_elements_font_added:', 'visualmodo' ) . $this->ip_name );
		}
		...
		function remove_zipped_font() {
			...
			if ( $delete ) {
				...
				die( esc_html__( 'visualmodo_elements_font_removed', 'visualmodo' ) );
			}
			die( esc_html__( 'Was not able to remove Icon Pack', 'visualmodo' ) );
		}
		...
		function zip_flatten( $zipfile, $filter ) {
			if ( is_dir( $this->paths['tempdir'] ) ) {
				$this->delete_folder( $this->paths['tempdir'] );
				$tempdir = visualmodo_elements_backend_create_folder( $this->paths['tempdir'], false );
			} else {
				$tempdir = visualmodo_elements_backend_create_folder( $this->paths['tempdir'], false );
			}
			//$fontdir = visualmodo_elements_backend_create_folder($this->paths['fontdir'], false);
			if ( ! $tempdir ) {
				die( esc_html__( 'Could not create temporary folder.', 'visualmodo' ) );
			}
			$zip = new ZipArchive;
			if ( $zip->open( $zipfile ) ) {
				for ( $i = 0; $i < $zip->numFiles; $i ++ ) {
					...
					if ( ! $fp ) {
						die( esc_html__( 'Unable to unpack Icon Pack.', 'visualmodo' ) );
					}
					while ( ! feof( $fp ) ) {
				...
				die( esc_html__( "Icon Pack Zip file is corrupted.", 'visualmodo' ) );
			}
		...
		function create_config() {
			...
			if ( empty( $this->json_file ) || empty( $this->svg_file ) ) {
				$this->delete_folder( $this->paths['tempdir'] );
				die( esc_html__( 'SVG file or selection.json not found. Please check the integrity of the Icon Pack files.', 'visualmodo' ) );
			}
			...
			if ( ! is_wp_error( $json ) && ! empty( $json ) ) {
				...
				if ( is_dir( $font_folder ) ) {
					$this->delete_folder( $this->paths['tempdir'] );
					die( esc_html__( "There is already an Icon Pack with this name, add an icon with another name.", "visualmodo" ) );
				}
				$file_contents = json_decode( $json );
				if ( ! isset( $file_contents->IcoMoonType ) ) {
					$this->delete_folder( $this->paths['tempdir'] );
					die( esc_html__( "This Icon Pack does not belong to Icomoon. Use only Icomoon's Icon Pack.", 'visualmodo' ) );
				}
			...
			if ( $handle ) {
				...
				foreach ( $this->json_config[ $this->ip_name ] as $icon => $info ) {
					if ( ! empty( $info ) ) {
						...
						$this->delete_folder( $this->paths['tempdir'] );
						die( esc_html__( 'Error generating configuration file.', 'visualmodo' ) );
					}
				}
				fclose( $handle );
			} else {
		...
		function re_write_css() {
			...
			if ( $file ) {
				...
				$str = str_replace( '.icon {', '[class^="' . $this->ip_name . '-"], [class*=" ' . $this->ip_name . '-"] {', $str );
					$str = str_replace( 'i {', '[class^="' . $this->ip_name . '-"], [class*=" ' . $this->ip_name . '-"] {', $str );
						...
						die( esc_html__( "Error generating CSS file. Use only Icomoon's Icon Pack.", 'visualmodo' ) );
					}
				}
				...
				function rename_folder() {
					...
					if ( rename( $this->paths['tempdir'], $new_name ) ) {
						...
						$this->delete_folder( $this->paths['tempdir'] );
						die( esc_html__( "This Icon Pack could not be added.", "visualmodo" ) );
					}
				}
				...
				function add_font() {
					...
					update_option( 'visualmodo_elements_ip', $fonts );
				}
				...
				static function load_iconfont_list() {
					...
					$extra_fonts = get_option( 'visualmodo_elements_ip' );
					if ( empty( $extra_fonts ) ) {
						$extra_fonts = array();
					}
					$font_configs = $extra_fonts;//array_merge(visualmodoElementsBuilder::$default_iconfont, $extra_fonts);
			...
			*/
			...
			if ( ! function_exists( 'visualmodo_elements_backend_create_folder' ) ) {
				function visualmodo_elements_backend_create_folder( &$folder, $addindex = true ) {
			...
			new VISUALMODO_ELEMENTS_IPM;
		}
		
		add_action( 'admin_menu', 'visualmodo_elements_icon_manager_menu', 99 );
		
		if ( ! function_exists( 'visualmodo_elements_icon_manager_menu' ) ) {
			function visualmodo_elements_icon_manager_menu() {
				$icon_manager_page = add_submenu_page(
					'visualmodo-elements.php',
					esc_html__( "Icon Pack Manager", "visualmodo" ),
					esc_html__( "Icon Pack Manager", "visualmodo" ),
					"manage_options",
					"icon-manager",
					"visualmodo_elements_custom_icons_menu"
				);
				$VISUALMODO_ELEMENTS_IPM  = new VISUALMODO_ELEMENTS_IPM;
				add_action('admin_print_styles-' . $icon_manager_page, array( $VISUALMODO_ELEMENTS_IPM, 'admin_scripts' ) );
			}
		}
		
		function visualmodo_elements_custom_icons_menu() {
			$VISUALMODO_ELEMENTS_IPM = new VISUALMODO_ELEMENTS_IPM;
			$VISUALMODO_ELEMENTS_IPM->icon_pack_manager();
		}
		
		function visualmodo_elements_custom_icons() {
			$upload_paths = wp_upload_dir();
			$custom_fonts = get_option( 'visualmodo_elements_ip' );
			if ( is_array( $custom_fonts ) ) {
				foreach ( $custom_fonts as $font => $info ) {
		...
		add_action( 'wp_enqueue_scripts', 'visualmodo_elements_custom_icons' );